...
\else
  \author{%
    \IEEEauthorblockN{1\textsuperscript{st} Alexis Montoison}
    \IEEEauthorblockA{
        \textit{Mathematics and Computer Science Division} \\
        \textit{Argonne National Laboratory}\\
        Lemont, IL, USA \\
        amontoison@anl.gov
    }
    \and
    \IEEEauthorblockN{2\textsuperscript{nd} Jean-Baptiste Caillau}
    \IEEEauthorblockA{
        \textit{Universit\'e C\^ote d'Azur, CNRS, Inria, LJAD} \\
        Nice, France \\
        jean-baptiste.caillau@univ-cotedazur.fr
    }
  }
\fi
...
\begin{abstract}
    ...
    The direct transcription of ODE dynamics produces block-separable constraints with identical computational patterns across time steps.
    This regularity aligns naturally with SIMD execution on GPUs, enabling efficient batched evaluation of constraint and derivative computations.
    These programs are compiled into GPU kernels using \texttt{ExaModels.jl} for fast evaluation of objectives, gradients, Jacobians, and Hessians.
    The resulting sparse problems are solved entirely on GPU using the interior-point solver \texttt{MadNLP.jl} together with the GPU sparse linear solver cuDSS, yielding significant speed-ups over CPU-based approaches.
\end{abstract}
...
\end{figure}
 ...
 \begin{figure}
\includegraphics[width=.45\textwidth]{goddard-h100-n2.jpg}
\caption{Goddard problem, H100 solve. The grid size ranges from to $N = 1e2$ to $N = 2e5$.}
\label{fig3}
\end{figure}
 
\begin{figure}
\includegraphics[width=.45\textwidth]{quadrotor-a100-n2.png}
\caption{Quadrotor problem, A100 solve. The grid size ranges from to $N = 1e2$ to $N = 1e5$.}
\label{fig2}
\end{figure}

\begin{figure}
\includegraphics[width=.45\textwidth]{quadrotor-h100-n.png}
